...
@Component({
    selector: 'jhi-batch-bli',
    templateUrl: './batch-bli.component.html'
})
export class BatchBliComponent implements OnInit, OnDestroy {
...
currentAccount: any;
    ...
    loadAll() {
        this.batchService.query({
            ...
            sort: this.sort()}).subscribe(
                (res: HttpResponse<BatchBli[]>) => this.onSuccess(res.body, res.headers),
    ...
    transition() {
        this.router.navigate(['/batch-bli'], {queryParams:
            {
                page: this.page,
                size: this.itemsPerPage,
    ...
    clear() {
        this.page = 0;
        this.router.navigate(['/batch-bli', {
            page: this.page,
            sort: this.predicate + ',' + (this.reverse ? 'asc' : 'desc')
        }]);
        this.loadAll();
    ...
    trackId(index: number, item: BatchBli) {
        return item.id;
    }
